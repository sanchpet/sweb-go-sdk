// Package ssl groups VPS SSL-certificate operations (endpoint /vps/ssl): list
// the account's certificates, browse and order the certificate catalogue,
// download an issued certificate archive, and manage a certificate's
// auto-prolongation and lifecycle. All calls dispatch through the shared
// transport.
package ssl
...
// Service groups VPS SSL-certificate operations (endpoint /vps/ssl): list the
...
type Certificate struct {
	...
	ProlongAvailable   flex.Int `json:"prolong_available"` // 1 if prolongation is offered
...
func (s *Service) OrderSubmit(ctx context.Context, domain string, certificateID int, confirmMail string, opts *OrderSubmitOptions) (json.RawMessage, error) {
	if opts == nil {
	...
	var out json.RawMessage
	err := s.t.Call(ctx, sslEndpoint, "orderSubmit", p, &out)
	return out, err
}
