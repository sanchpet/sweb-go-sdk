...
func TestBalancerList(t *testing.T) {
	...
	list, err := c.Balancer.List(context.Background())
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	if gotMethod != "index" {
...
func TestBalancerIsCreateEnable(t *testing.T) {
	for _, tc := range []struct {
		...
		got, err := c.Balancer.IsCreateEnable(context.Background())
		if err != nil {
...
func TestBalancerAvailableConfig(t *testing.T) {
	...
	cfg, err := c.Balancer.AvailableConfig(context.Background())
	if err != nil {
...
func TestBalancerCreate(t *testing.T) {
	...
	err := c.Balancer.Create(context.Background(), BalancerCreateOptions{
		Datacenter:   1,
		Type:         "leastconn",
		Servers:      []BalancerServer{{IP: "1.1.1.1"}},
		Rules:        []BalancerRule{{ProtocolBalancer: "https", PortBalancer: "443", ProtocolServer: "https", PortServer: "443"}},
		PlanID:       4298,
		SaveSession:  true,
		Alias:        "LB 3",
		IsFirstOrder: false,
...
func TestBalancerCreateAliasOmittedWhenEmpty(t *testing.T) {
	...
	if err := c.Balancer.Create(context.Background(), BalancerCreateOptions{Type: "leastconn"}); err != nil {
		t.Fatalf("Create: %v", err)
	}
	if hasAlias {
...
func TestBalancerEdit(t *testing.T) {
	...
	err := c.Balancer.Edit(context.Background(), BalancerEditOptions{
		BillingID:  "dyasyuc578_balancer_1",
		Type:       "roundrobin",
		Servers:    []BalancerServer{{IP: "2.2.2.2", Weight: 3}},
		Rules:      []BalancerRule{{ProtocolBalancer: "http", PortBalancer: "80", ProtocolServer: "http", PortServer: "80"}},
		ProxyProto: true,
		Keepalive:  true,
	})
	if err != nil {
...
func TestBalancerRemove(t *testing.T) {
	...
	if err := c.Balancer.Remove(context.Background(), "dyasyuc578_balancer_1"); err != nil {
...
func TestBalancerSentinelFailure(t *testing.T) {
	...
	if err := c.Balancer.Remove(context.Background(), "x"); err == nil {
