...
func TestList(t *testing.T) {
	...
	domains, err := s.List(context.Background(), &ListOptions{Type: "all"})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
...
func TestInfo(t *testing.T) {
	...
	info, err := s.Info(context.Background(), "example.com")
	if err != nil {
		t.Fatalf("Info: %v", err)
	}
...
func TestInfoNilProlongConfirm(t *testing.T) {
...
func TestSubdomains(t *testing.T) {
	...
	subs, err := s.Subdomains(context.Background(), "example.com")
	if err != nil {
...
// TestAvailablePackages covers the quoted-string numerics (id/price/priority
// arrive as strings, decoded through flex.Int/flex.Float).
func TestAvailablePackages(t *testing.T) {
	...
	pkgs, err := s.AvailablePackages(context.Background(), "example.ru", "example.shop")
	if err != nil {
...
func TestQueryFlags(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ok, err := s.RegAvailable(context.Background(), "example.com", PayBalance)
			if err != nil {
			...
			ok, err = s.TransferAvailable(context.Background(), "example.com")
			if err != nil {
...
func TestRegistrationPrice(t *testing.T) {
	s := serve(t, func(w http.ResponseWriter, _ *http.Request) {
		// Doc types it as string; the example returns a bare number. flex.Float takes both.
		_, _ = w.Write([]byte(`{"jsonrpc":"2.0","result":199}`))
	})
	price, err := s.RegistrationPrice(context.Background(), "example.com")
	if err != nil {
...
// TestActionOne covers the 1=success mutating methods and the non-1 failure.
func TestActionOne(t *testing.T) {
	...
	if err := s.ChangeProlong(context.Background(), "example.com", ProlongBonusMoney); err != nil {
...
func TestActionOneFailure(t *testing.T) {
	...
	if err := s.Remove(context.Background(), "example.com"); err == nil {
...
func TestMoveSuccess(t *testing.T) {
	...
	if err := s.Move(context.Background(), "example.com", ProlongBonusMoney, "/home/e/example"); err != nil {
		t.Fatalf("Move: %v", err)
	}
	if gotMethod != "move" {
...
func TestMoveListParams(t *testing.T) {
	...
	err := s.MoveList(context.Background(),
...
// TestProlongList covers the always-wrapped {"extendedResult":{…}} envelope.
func TestProlongList(t *testing.T) {
	s := serve(t, func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"jsonrpc":"2.0","result":{"extendedResult":` +
			`{"code":1,"message":"domains prolonged","data":[]}}}`))
	})
	er, err := s.ProlongList(context.Background(), "a.com", "b.com")
	if err != nil {
...
func TestProlongListFailure(t *testing.T) {
	s := serve(t, func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"jsonrpc":"2.0","result":{"extendedResult":` +
			`{"code":0,"message":"insufficient balance","data":[]}}}`))
	})
	er, err := s.ProlongList(context.Background(), "a.com")
	if err == nil {
...
func TestSubdomainAndRedirect(t *testing.T) {
	for _, tc := range []struct {
		name, method string
		call         func(*Service) error
	}{
		{"CreateSubdomain", "createSubdomain", func(s *Service) error {
			return s.CreateSubdomain(context.Background(), "example.com", "test1", "/test")
		}},
		{"RemoveSubdomain", "removeSubdomain", func(s *Service) error {
			return s.RemoveSubdomain(context.Background(), "example.com", "test1")
		}},
		{"SetRedirect", "setRedirectVh", func(s *Service) error {
			return s.SetRedirect(context.Background(), "example.com", "https://example.org")
		}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			var gotMethod string
...
func TestGetRedirect(t *testing.T) {
	...
	url, err := s.Redirect(context.Background(), "example.com")
	if err != nil {
...
// serve spins up a mock JSON-RPC server for h and returns a domains.Service
