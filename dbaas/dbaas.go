// Package dbaas groups managed-database (DBaaS) operations (endpoint /dbaas):
...
// Service groups managed-database (DBaaS) operations (endpoint /dbaas):
// listing clusters, the create-page config/constructor lookups, the promotional
// first-order flow, and cluster/database lifecycle (create/edit/remove/delete).
...
// Index is the object returned by List (method "index"): the account's
// clusters plus beta-quota metadata.
type Index struct {
	Instances    []Instance `json:"instances"`
	TotalCount   flex.Int   `json:"total_count"` // arrives as a quoted string
	MaxCount     flex.Int   `json:"max_count"`
	CanCreate    bool       `json:"can_create"`
	UpgradeAgree *bool      `json:"upgrade_agree"` // null until the user chooses
}

// Instance is one managed-database cluster in Index.Instances.
type Instance struct {
	...
	Instances     flex.Int   `json:"instances"`
	SyncReplicas  flex.Int   `json:"sync_replicas"`
	ReadReplicas  flex.Int   `json:"read_replicas"`
	Replicas      flex.Int   `json:"replicas"`
...
// Plan is a DBaaS tariff (shared by index and getAvailableConfig). Numeric
// fields arrive as quoted strings in observed responses.
type Plan struct {
	ID      flex.Int `json:"id"`
	Name    string   `json:"name"`
	CPU     flex.Int `json:"cpu"`
	Memory  flex.Int `json:"memory"`  // GB
	Storage flex.Int `json:"storage"` // GB
}

// Endpoint is a cluster connection endpoint. Type is "rw" (always present)
...
// User is a cluster or per-database user (only the name is exposed).
type User struct {
	Name string `json:"name"`
}

// Database is one database inside a cluster.
...
// "index"). Read-only.
func (s *Service) List(ctx context.Context) (*Index, error) {
	var out Index
	if err := s.t.Call(ctx, dbaasEndpoint, "index", nil, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
func (s *Service) SetUpgradeAgree(ctx context.Context, agree bool) error {
	return s.actionOne(ctx, "setUpgradeAgree", map[string]any{"upgradeAgree": agree})
}

// Config is the create-page config (method "getAvailableConfig"): available
...
// Engine is one selectable DBMS version in Config.Engines.
type Engine struct {
	Name    string `json:"name"`
...
func (s *Service) AvailableConfig(ctx context.Context) (*Config, error) {
	var out Config
	if err := s.t.Call(ctx, dbaasEndpoint, "getAvailableConfig", nil, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
// Read-only.
func (s *Service) ConstructorPlanID(ctx context.Context, cpu, memory, storage, replicas int) (int64, error) {
	var out flex.Int
	err := s.t.Call(ctx, dbaasEndpoint, "getConstructorPlanId", map[string]any{
		"cpu":      cpu,
		"memory":   memory,
		"storage":  storage,
		"replicas": replicas,
...
// FirstOrder describes the account's promotional first DBaaS order (method
...
// bare (replicas, instances, pay_period) — all decoded through flex.Int. Nullable
// per the doc; PricePerMonth is flex.Float (doc says int but the example returns a
...
func (s *Service) GetFirstOrderInfo(ctx context.Context) (*FirstOrder, error) {
	var out FirstOrder
	if err := s.t.Call(ctx, dbaasEndpoint, "getFirstOrderInfo", nil, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
// struct — the transport already surfaces a JSON-RPC error object as *apierr.Error.
func (s *Service) RemoveFirst(ctx context.Context) (json.RawMessage, error) {
	var out json.RawMessage
	err := s.t.Call(ctx, dbaasEndpoint, "removeFirst", nil, &out)
	return out, err
}

// UserCredentials is a cluster user in a create/edit/validate request. Omit
...
// struct. A JSON-RPC error surfaces as *apierr.Error from the transport.
func (s *Service) CreateInstance(ctx context.Context, req CreateInstanceRequest) (json.RawMessage, error) {
	var out json.RawMessage
	err := s.t.Call(ctx, dbaasEndpoint, "createInstance", req, &out)
	return out, err
}
...
type EditInstanceRequest struct {
	...
	Users           []UserCredentials `json:"users,omitempty"`
...
func (s *Service) EditInstance(ctx context.Context, req EditInstanceRequest) error {
...
// as a guessed sentinel. A JSON-RPC error surfaces as *apierr.Error from the transport.
func (s *Service) RemoveInstance(ctx context.Context, billingID string) (json.RawMessage, error) {
	var out json.RawMessage
	err := s.t.Call(ctx, dbaasEndpoint, "removeInstance", map[string]string{"billingId": billingID}, &out)
	return out, err
}

// DeleteDatabase removes a single database from a cluster (method
// "deleteDatabase"). DESTRUCTIVE. dbName is a Database.Name from List.
//
// Evidence-first: the documented result is a Success message wrapped as
...
// error surfaces as *apierr.Error from the transport.
func (s *Service) DeleteDatabase(ctx context.Context, billingID, dbName string) (json.RawMessage, error) {
	var out json.RawMessage
	err := s.t.Call(ctx, dbaasEndpoint, "deleteDatabase", map[string]string{
		"billingId": billingID,
		"dbName":    dbName,
	}, &out)
	return out, err
...
// success or a validation error message otherwise (surfaced as *apierr.Error).
func (s *Service) ValidateUsers(ctx context.Context, users []UserCredentials) error {
	var out bool
	if err := s.t.Call(ctx, dbaasEndpoint, "validateUsers", map[string]any{"users": users}, &out); err != nil {
		return err
	}
	if !out {
...
func (s *Service) actionOne(ctx context.Context, method string, params any) error {
	var out flex.Int
	if err := s.t.Call(ctx, dbaasEndpoint, method, params, &out); err != nil {
		return err
	}
	if out != 1 {
