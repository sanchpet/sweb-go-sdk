// Package transport is the internal JSON-RPC spine of the SDK: it owns the
// Client (HTTP transport, envelope, Bearer auth with transparent token refresh,
// and error mapping) and the functional Options that configure it. Service
// packages dispatch through transport.Client.Call; the root sweb package wires
// them over one shared Client.
//
// It lives under internal/ so the transport/auth/retry boundary is
// compiler-enforced: consumers outside the module cannot import it.
package transport
...
// It is the mechanism behind the facade's CreateToken.
...
func (c *Client) doCall(ctx context.Context, endpoint, method string, params, out any) error {
	...
	if resp.StatusCode != http.StatusOK {
		return &apierr.Error{Code: resp.StatusCode, Message: fmt.Sprintf("http %s: %s", resp.Status, truncate(body, 256))}
	}
