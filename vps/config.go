...
// Plan is a purchasable VPS plan. Note SpaceWeb returns several numeric-ish
...
func (s *Service) AvailableConfig(ctx context.Context) (*AvailableConfig, error) {
	var out AvailableConfig
	if err := s.t.Call(ctx, vpsEndpoint, "getAvailableConfig", nil, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
